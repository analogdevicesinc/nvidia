...
print_help(void)
{
    cerr <<
        ...
        "\t-outchromaloc <horiz> <vert> Sets the ChromaLoc value for output [Default = 0 0]\n"
        "\t-inchromaloc <horiz> <vert> Sets the ChromaLoc value for input [Default = 0 0]\n\n"
        ...
        "2 = 5-tap      3 = 10-tap\n"
        "4 = smart      5 = nicest\n\n"
        "Allowed values for chromaLocHoriz and chromaLocVert:\n"
...
get_color_format(const char* userdefined_fmt)
{
    ...
    if (!strcmp(userdefined_fmt, "UYVY_709"))
        return NVBUF_COLOR_FORMAT_UYVY_709;
    if (!strcmp(userdefined_fmt, "UYVY_709_ER"))
        return NVBUF_COLOR_FORMAT_UYVY_709_ER;
    if (!strcmp(userdefined_fmt, "UYVY_2020"))
    ...
    if (!strcmp(userdefined_fmt, "BGRA"))
...
parse_csv_args(context_t * ctx, int argc, char *argv[])
{
    ...
    while ((arg = *(++argp)))
    {
        ...
        else if (!strcmp(arg, "-outchromaloc"))
        {

            argp++;
            CHECK_OPTION_VALUE(argp);
            ctx->out_chroma_loc.horiz = atoi(*argp);
            CSV_PARSE_CHECK_ERROR((ctx->out_chroma_loc.horiz < 0 ||
                    ctx->out_chroma_loc.horiz > 2),
                    "outchromaloc horiz out of bounds");

            argp++;
            CHECK_OPTION_VALUE(argp);
            ctx->out_chroma_loc.vert = atoi(*argp);
            CSV_PARSE_CHECK_ERROR((ctx->out_chroma_loc.vert < 0 ||
                    ctx->out_chroma_loc.vert > 2),
                    "outchromaloc vert out of bounds");

        }
        else if (!strcmp(arg, "-inchromaloc"))
        {

            argp++;
            CHECK_OPTION_VALUE(argp);
            ctx->in_chroma_loc.horiz = atoi(*argp);
            CSV_PARSE_CHECK_ERROR((ctx->in_chroma_loc.horiz < 0 ||
                    ctx->in_chroma_loc.horiz > 2),
                    "inchromaloc horiz out of bounds");

            argp++;
            CHECK_OPTION_VALUE(argp);
            ctx->in_chroma_loc.vert = atoi(*argp);
            CSV_PARSE_CHECK_ERROR((ctx->in_chroma_loc.vert < 0 ||
                    ctx->in_chroma_loc.vert > 2),
                    "inchromaloc vert out of bounds");
        }
        else
        {
